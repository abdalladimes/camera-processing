...
function getResultsFromExtractedColors(extractedColors, parameters) {
    const results = {};
    for (const parameterKeyFromResults in extractedColors) {
        const parameterColorFromResults = extractedColors[parameterKeyFromResults];
        const parameter = parameters[parameterKeyFromResults];
        if (parameter) {
            const parameterValueRanges = parameter.valueRanges;
            let index = 0;
            for (const valueRange of parameterValueRanges) {
                index = parameterValueRanges.indexOf(valueRange);
                const referenceColor = valueRange.referenceColor;
                const referenceColorRgb = hexToRgba(referenceColor.hex); 
                if (isColorMatching(parameterColorFromResults, referenceColorRgb, parameter.tolerance)) {
                    results[parameterKeyFromResults] = {
                        value: valueRange.correspondingValue,
                        interpretation: valueRange.interpretation,
                        clarification: valueRange.clarification,
                        referenceColor: referenceColor.hex,
                        referenceColorRgb: referenceColorRgb
                    }
                    break;
                } else {
                    if (index === parameterValueRanges.length - 1) {
                        getOutOfRangeResult();
                    }
                }
            }
...
function isColorMatching(color1, color2, tolerance = 30) {
...
// 1. Helper function to convert RGB to Hex for nice labels
